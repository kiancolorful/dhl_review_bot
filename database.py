...
def fetch_incomplete_reviews(engine, count : int=None) -> pandas.DataFrame: # These reviews were answered on kununu before the bot got to them. The goal of this is to fill out the extra information columns for these. 
    try:
        if count:
            df = pandas.read_sql(f"SELECT TOP {count} * FROM {SQL_TABLE_NAME} WHERE NOT (Response='' OR Response IS NULL) AND SensitiveTopic IS NULL", engine)
        else:
